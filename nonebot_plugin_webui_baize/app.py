...
def create_app(start_time: float, data_dir: Path) -> FastAPI:
    ...
    async def get_plugins():
        ...
        # 2b) 目录插件 (xxx/__init__.py) — 排除已知非插件包
    ...
    _persona_local_plugins = _get_plugins_dir()  # 本地 plugins 目录
    ...
    _local_plugins = _get_plugins_dir()
    ...
    for _help_mod in ("src.plugins.nonebot_plugin_help_baize", "nonebot_plugin_help_baize"):
        try:
            _help_webui_api = __import__(f"{_help_mod}.webui_api", fromlist=["router"])
