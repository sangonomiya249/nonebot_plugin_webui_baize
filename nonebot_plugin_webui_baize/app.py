...
def create_app(start_time: float, data_dir: Path) -> FastAPI:
    ...
    async def update_plugin_config(request: Request):
        ...
        # --- Python 常量更新 ---
        if changes:
            if _update_plugin_config(file_path, changes):
                logger.info(f"插件配置已更新: {module} → {changes}")
                # 尝试触发插件运行时重载配置
                try:
    ...
    # 在候选目录中查找 persona_prompts.json
    if not _PERSONA_PROMPTS_PATH:
        for _search_dir in _PERSONA_SEARCH_DIRS:
            _candidate_path = _search_dir / "persona_prompts.json"
    ...
    # 最终兜底
    if not _PERSONA_PROMPTS_PATH:
        _PERSONA_PROMPTS_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)) or ".", "persona_prompts.json")
    ...
    # 在所有候选目录中查找 token_daily.json
    for _search_dir in _TOKEN_DAILY_SEARCH_DIRS:
        _candidate_path = _search_dir / "token_daily.json"
        if _candidate_path.is_file():
