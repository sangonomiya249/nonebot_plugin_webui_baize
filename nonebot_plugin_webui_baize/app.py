...
def create_app(start_time: float, data_dir: Path) -> FastAPI:
    ...
    if not _AICHAT_AVAILABLE:
        try:
            ...
            pass
    # 5) 本地目录存在性兜底（优先 CWD，再本文件相邻）
    if not _AICHAT_AVAILABLE:
        _local_aichat = Path.cwd() / "src" / "plugins" / "nonebot_plugin_aichat_baize"
        if not (_local_aichat.is_dir() and (_local_aichat / "__init__.py").is_file()):
    ...
    async def get_plugins():
        ...
        # 2b) 目录插件 (xxx/__init__.py) — 遍历所有插件目录，排除已知非插件包
        for plugins_dir in all_plugin_dirs:
            for d in sorted(plugins_dir.iterdir()):
                if not d.is_dir() or d.name.startswith("_") or d.name.startswith("."):
                    continue
                if d.name in _SKIP_PACKAGES:
                    continue
                # 当扫描 site-packages 时，只保留 nonebot_plugin_ 或 haruka_ 前缀的包
                if _is_external_plugin(d) and not (
                    d.name.startswith("nonebot_plugin_") or d.name.startswith("haruka_bot_")
                ):
                    continue
        ...
        # 4) 扫描禁用的插件（.py.disabled / .disabled 目录）— 遍历所有插件目录
        for plugins_dir in all_plugin_dirs:
    ...
    async def update_plugin_config(request: Request):
        ...
        allowed_dir = _get_plugins_dir().resolve()
    ...
    async def install_github_plugin(request: Request):
        ...
        plugins_dir = _get_plugins_dir()  # bot 的 plugins 目录
    ...
    _persona_plugin_dirs = _get_all_plugin_dirs()  # 全部插件目录
    ...
    # 4) 本地目录查找 — 遍历所有插件目录
    for _dir in _persona_plugin_dirs:
    ...
    # 5) glob 兜底 — 遍历所有插件目录
    for _dir in _persona_plugin_dirs:
        for _candidate in _dir.glob("**/persona_prompts.json"):
    ...
    # 最终兜底：优先用已找到的 search dir，其次回退到第一个插件目录
    if not _PERSONA_PROMPTS_PATH:
        if _PERSONA_SEARCH_DIRS:
            _PERSONA_PROMPTS_PATH = str(_PERSONA_SEARCH_DIRS[0] / "persona_prompts.json")
        elif _persona_plugin_dirs:
            _PERSONA_PROMPTS_PATH = str(_persona_plugin_dirs[0] / "nonebot_plugin_aichat_baize" / "persona_prompts.json")
    ...
    _token_plugin_dirs = _get_all_plugin_dirs()  # 全部插件目录
    ...
    # 4) 本地插件目录直接查找 — 遍历所有插件目录
    for _dir in _token_plugin_dirs:
        for _sub in ("nonebot_plugin_aichat_baize", "nonebot_plugin_deepseek_chat/nonebot_plugin_aichat_baize"):
            _d = _dir / _sub
            if _d.is_dir() and _d not in _TOKEN_DAILY_SEARCH_DIRS:
                _TOKEN_DAILY_SEARCH_DIRS.append(_d)

    # 5) glob 兜底搜索 — 遍历所有插件目录
    for _dir in _token_plugin_dirs:
        for _candidate in _dir.glob("**/token_daily.json"):
            _cand_dir = _candidate.parent
    ...
    # ---- 帮助模块管理 API（通过 sys.modules 动态发现，适配任意目录布局） ----
    ...
    try:
        # 策略 1：从 sys.modules 找（已加载的插件必定在此）
        for _key, _mod in list(sys.modules.items()):
            if _key.endswith(".nonebot_plugin_help_baize") or _key == "nonebot_plugin_help_baize":
                _help_mod_name = _key
                _help_mod = _mod
                break

        # 策略 2：兜底尝试标准 import 路径
        if not _help_mod_name:
            for _candidate in ("src.plugins.nonebot_plugin_help_baize", "nonebot_plugin_help_baize"):
                try:
                    _help_mod = __import__(_candidate)
                    _help_mod_name = _candidate
                    break
                except ImportError:
                    continue
        ...
        if not _help_mod_name:
            ...
            for _dir in _get_all_plugin_dirs():
                _help_dir = _dir / "nonebot_plugin_help_baize"
                _init = _help_dir / "__init__.py"
                if _init.is_file():
                    try:
                        _spec = _iu.spec_from_file_location(
                            "nonebot_plugin_help_baize",
                            str(_init),
                            submodule_search_locations=[str(_help_dir)],
                        )
                        if _spec and _spec.loader:
                            _help_mod = _iu.module_from_spec(_spec)
                            sys.modules["nonebot_plugin_help_baize"] = _help_mod
                            _spec.loader.exec_module(_help_mod)
                            _help_mod_name = "nonebot_plugin_help_baize"
                            break
                    except Exception:
                        continue

        # 加载 webui_api 路由
        if _help_mod is not None:
            try:
                _help_webui_api = __import__(f"{_help_mod_name}.webui_api", fromlist=["router"])
                _help_router = _help_webui_api.router
                _HELP_AVAILABLE = True
                if hasattr(_help_mod, "__file__") and _help_mod.__file__:
                    _help_data_dir = Path(_help_mod.__file__).parent / "data"
                logger.info(f"帮助模块已挂载: {_help_mod_name}")
            except ImportError as _e:
                logger.warning(f"帮助模块 webui_api 导入失败 ({_help_mod_name}): {_e}")
        else:
            logger.warning("帮助模块未找到，WebUI 帮助管理功能不可用")
    except Exception as _e:
        logger.warning(f"帮助模块加载失败（不影响 WebUI 正常使用）: {_e}")
