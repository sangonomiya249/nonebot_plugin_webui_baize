...
__plugin_meta__ = PluginMetadata(
    ...
    usage=(
        "Web UI 管理面板\n"
        "发送「webui start」 启动 Web 服务器\n"
        "发送「webui stop」  停止 Web 服务器\n"
        "发送「webui status」查看 Web 服务器状态\n"
        "发送「webui」       获取访问地址\n\n"
        "默认地址: http://127.0.0.1:8899\n"
        "首次使用请发送「webui start」启动服务"
    ),
...
DATA_DIR = Path("data") / "web_ui"  # Web UI 数据目录
...
from .config_editor import _parse_plugin_matchers  # noqa: E402

# ==================== 日志收集器（委托给共享模块 log_bridge）====================

from src.plugins._log_bridge import (  # noqa: E402
    add_msg_recv,
    add_msg_sent,
    add_user_msg,
