...
class _EarlyLogHandler(logging.Handler):
    ...
    # NoneBot 框架内部噪音日志（不限级别，全部静音，与 loguru sink 保持一致）
    _NONEBOT_NOISE_PATTERNS_STDLIB = [
        re.compile(r"Running PreProcessors"),
        re.compile(r"Running PostProcessors"),
        re.compile(r"Running CalledAPI hooks"),
        re.compile(r"Running Matcher\("),
        re.compile(r"Running handler "),
        re.compile(r"Matcher\(.+running complete"),
        re.compile(r"Event will be handled by Matcher"),
        re.compile(r"Stop event propagation"),
        re.compile(r"Checking for matchers"),
        re.compile(r"OneBot V11 \| Calling API "),
        re.compile(r"OneBot V11 \d+ \| "),
    ]
    ...
    def emit(self, record: logging.LogRecord):
        try:
            ...
            # 静音 nonebot 框架内部噪音（不限级别）
            if name == "nonebot" and msg:
                if any(p.search(msg) for p in self._NONEBOT_NOISE_PATTERNS_STDLIB):
                    return
...
def setup():
    ...
    if nb_logger is not None:
        ...
        if not hasattr(setup, sink_attr):
            # NoneBot 框架内部噪音日志（不限级别，全部静音）
            _NONEBOT_NOISE_PATTERNS = [
                # 事件处理流水线
                re.compile(r"Running PreProcessors"),
                re.compile(r"Running PostProcessors"),
                re.compile(r"Running CalledAPI hooks"),
            ...
            def _loguru_sink(message):
                try:
                    ...
                    # 静音 uvicorn/fastapi 低于 WARNING 的日志
                    if name.startswith(("uvicorn.", "fastapi.")) and level_no < 30:
                        return
                    # 静音已知噪音 logger
                    for silenced, min_level in _EarlyLogHandler.SILENCED_LOGGERS.items():
                        if name.startswith(silenced) and level_no < min_level:
                            return
                    # 静音 nonebot 框架内部噪音（不限级别）
                    if name == "nonebot":
                        if any(p.search(msg) for p in _NONEBOT_NOISE_PATTERNS):
                            return

                    _append_log(level_name, f"{name} | {msg}")
                except Exception:
                    pass
