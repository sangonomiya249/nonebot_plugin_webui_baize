...
def _get_plugins_dir() -> Path:
    """获取 Bot 的主插件目录（兼容 pip 安装和本地安装两种部署方式）。

    优先级：
    1. CWD/src/plugins  — 最常见的 NoneBot 项目布局
    2. CWD/plugins       — 简化项目布局
    3. Path(__file__).parent.parent — webui 自身所在的本地目录（本地安装时就是 plugins/）
    """
    cwd = Path.cwd()
    for candidate in (
        cwd / "src" / "plugins",
        ...
        if candidate.is_dir():
            return candidate
    # 兜底：webui 本地安装时所在的目录（如 src/plugins/）
