...
def _get_plugins_dir() -> Path:
    ...
    始终返回 webui 自身所在目录，确保下载的插件和 webui 在一起。
    """
    return _get_own_dir()


def _get_all_plugin_dirs() -> list:
    """获取所有已发现的插件目录（用于扫描插件列表、指令、帮助等）。

    优先级：
    1. webui 自身所在目录（最高优先级）
    2. CWD/src/plugins
    3. CWD/plugins
    （自动去重）
    """
    ...
    dirs.append(own)
    cwd = Path.cwd()
    for candidate in (cwd / "src" / "plugins", cwd / "plugins"):
...
# 已知不该被当作插件的 Python 标准库 / 通用工具包
_SKIP_PACKAGES: set = {
    # 标准库相关
    "yaml", "_yaml", "json", "markdown", "PIL", "Pillow", "cv2",
    # 通用工具
    "pip", "setuptools", "pkg_resources", "wheel", "distutils",
    "certifi", "charset_normalizer", "idna", "urllib3", "requests",
    "six", "dateutil", "pytz", "tzdata",
    "numpy", "pandas", "scipy", "matplotlib",
    "attr", "attrs", "packaging", "pyparsing",
    "colorama", "tomli", "toml",
    # NoneBot 框架自身（非插件）
    "nonebot", "nb_cli", "fastapi", "uvicorn", "starlette",
    "httpx", "httpcore", "h11", "h2", "anyio", "sniffio",
    "click", "typing_extensions", "exceptiongroup",
}
"""已知的非插件包名（当扫描 site-packages 时会被自动过滤）"""


def _resolve_plugin_file(module: str) -> Optional[Path]:
    """定位插件的主 .py 文件（本地插件 + pip 外部插件）。

    搜索顺序：webui 所在目录 → CWD/src/plugins → CWD/plugins → sys.modules → find_spec
    """
    clean_name = module.rsplit(".", 1)[-1] if "." in module else module

    # 1) 遍历所有已发现的插件目录
    for plugins_dir in _get_all_plugin_dirs():
        # 单文件: xxx.py
        f = (plugins_dir / f"{clean_name}.py")
        if f.is_file():
            return f
        # 目录包: xxx/__init__.py
        f = (plugins_dir / clean_name / "__init__.py")
        if f.is_file():
            return f

    # 2) pip 安装的外部插件：先查 sys.modules，再 importlib，最后 find_spec
    try:
