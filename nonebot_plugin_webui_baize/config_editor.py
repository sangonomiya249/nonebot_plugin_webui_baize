        ...
        if name == "CONFIG" and raw_value == "{":
            ...
            try:
                ...
                for l in dict_text.splitlines():
                    ...
                    if vraw.endswith(","):
                        vraw = vraw[:-1].strip()
                    # 如果是 _env("K", default) 函数调用，提取第二个参数
                    env_m = re.match(r'_env\s*\(\s*"[^"]*"\s*,\s*(.+)\)\s*$', vraw)
                    if env_m:
    ...
    if config_changes:
        for key, new_val in config_changes.items():
            if isinstance(new_val, str):
                # 先尝试 _env("K", "old") → _env("K", "new")
                p1 = re.sub(
                    rf'("{key}"\s*:\s*_env\("[^"]*"\s*,\s*")[^"]*("\s*\)\s*,)',
                    rf'\g<1>{new_val}\2',
                    text, count=1
                )
                if p1 != text:
                ...
                # 先尝试 _env("K", old_num) → _env("K", new_num)
                p1 = re.sub(
                    rf'("{key}"\s*:\s*_env\("[^"]*"\s*,\s*)[\d.eE+\-*/]+(\s*\)\s*,)',
                    rf'\g<1>{new_val}\2',
                    text, count=1
                )
                if p1 != text:
...
def _parse_config_py(file_path: Path) -> list:
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            # 尝试从 _env("KEY", default) 中提取默认值 —— 仅接受可 literal_eval 的字面量
            env_m = re.match(r'_env\s*\(\s*"[^"]*"\s*,\s*(.+)\)\s*$', raw_value)
            if env_m:
            ...
            # 尝试从 Path(_env("KEY", default)) 中提取
            elif (path_env_m := re.match(r'Path\s*\(\s*_env\s*\(\s*"[^"]*"\s*,\s*(.+)\)\s*\)\s*$', raw_value)):
...
def _update_config_py(file_path: Path, changes: dict) -> bool:
    ...
    for i, line in enumerate(lines):
        ...
        env_m = re.match(r'_env\s*\(\s*("[^"]*")\s*,\s*.+\)\s*$', orig_val)
        path_env_m = re.match(r'Path\s*\(\s*_env\s*\(\s*("[^"]*")\s*,\s*.+\)\s*\)\s*$', orig_val)
...
def _parse_plugin_config_full(file_path: Path) -> dict:
    ...
    if file_path.exists():
        try:
            ...
            text = ""
        # 查找指向 JSON 文件的变量（支持 "xxx.json" 和 PLUGIN_DIR / "xxx.json"）
        for m in re.finditer(
            r'([A-Z_]*FILE|[A-Z_]*PATH)\s*=\s*[^"\']*["\'](.+?\.json)["\']',
            text,
        ):
            json_rel = m.group(2)
            # 按优先级查找：项目根目录 → 插件目录 → data/ 子目录
            json_path = None
            candidates = [
